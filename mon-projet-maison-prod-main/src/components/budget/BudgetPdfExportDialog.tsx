...
type PdfType = "preliminary" | "actual" | "final";
...
export function BudgetPdfExportDialog({
  ...
  const buildPdf = (
    ...
    const title = type === "preliminary"
      ...
      : type === "final"
        ? t("budget.pdf.titleFinal", "Bilan final – Factures payées")
    ...
    const tableStartY = y;
    const rows: string[][] = [];
    const isActual = type === "actual" || type === "final";
    ...
    const fileName =
      (type === "preliminary" ? "budget-preliminaire-" : type === "final" ? "bilan-final-factures-payees-" : "budget-reel-") +
    ...
    try {
      ...
      if (saveToDossiers && projectId && userId) {
        ...
        if (storageError) {
          ...
          const taskId = pdfType === "preliminary" ? "bilan-preliminaire" : pdfType === "final" ? "bilan-final" : "bilan-reel";
          ...
          <RadioGroup value={pdfType} onValueChange={(v) => setPdfType(v as PdfType)} className="space-y-3">
            ...
            <div className="flex items-start space-x-3 border rounded-lg p-3">
              ...
              <Label htmlFor="final" className="flex-1 cursor-pointer">
                <span className="font-medium">{t("budget.pdf.optionFinal", "Bilan final : factures payées")}</span>
                <p className="text-sm text-muted-foreground mt-0.5">
                  {t("budget.pdf.optionFinalDesc", "Récapitulatif des factures payées par catégorie.")}
                </p>
              </Label>
            </div>
          </RadioGroup>
