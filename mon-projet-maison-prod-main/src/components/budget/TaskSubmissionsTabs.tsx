...
interface TaskSubmissionsTabsProps {
  ...
  /** Postes couverts par la soumission analysée (pour suivi restants à soumettre) */
  postesInclus?: Set<string>;
}
...
export function TaskSubmissionsTabs({
  ...
  // Suivi postes restants à soumettre
  const postesCouverts = postesInclus ? tasks.filter(task => postesInclus.has(task.taskTitle)).length : 0;
  const postesRestants = postesInclus ? tasks.filter(task => !postesInclus.has(task.taskTitle)).length : 0;
        ...
        <ScrollArea className="w-full">
          <TabsList className="inline-flex h-auto p-1 w-auto min-w-full flex-wrap gap-1">
            {tasks.map((task) => {
              ...
              const isCouvert = postesInclus?.has(task.taskTitle) ?? false;
              ...
              return (
                ...
                >
                  ...
                  {isCouvert && (
                    <CheckCircle2 className="h-3 w-3 text-primary shrink-0" title={t("budget.taskSubmissions.couvertParAnalyse", "Couvert par la soumission analysée")} />
                  )}
                  {hasSupplier && !isCouvert && (
                    <CheckCircle2 className="h-3 w-3 text-emerald-600 dark:text-emerald-400 shrink-0" />
                  )}
                  {!hasSupplier && hasDocuments && (
                    <FileText className="h-3 w-3 text-amber-500 shrink-0" />
                  )}
                  {!isCouvert && !hasSupplier && (
                    <span className="h-2 w-2 rounded-full bg-amber-500 shrink-0" title={t("budget.taskSubmissions.aSoumettre", "À soumettre")} />
                  )}
                  {taskTotal > 0 && (
                    <Badge 
                      variant="outline" 
